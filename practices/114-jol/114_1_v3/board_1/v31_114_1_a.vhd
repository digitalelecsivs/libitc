...
architecture arch of v31_114_1_a is
	--state machine
	...
	signal tts_init_flag : std_logic := '0';
	--user signal
	...
	signal sec_min : std_logic := '0';--sec:0 min: 1
	--pic signal
				...
				case mode_t is
					when mode00 =>
						case mode_1 is
							...
							when txt_mode =>
								...
								if txt_cnt = 3 then
									...
									if cnt_tim = '0' then
										...
										text_data(6 to 7) <= to_string(step, step'high, 10, 2);
										text_data(8 to 12) <= (others => character'val(32));
									elsif cnt_tim = '1' then
										text_data(1 to 5) <= "Time:";
										text_data(6 to 10) <= to_string(tim_min, tim_min'high, 10, 2) & ":" & to_string(tim_sec, tim_sec'high, 10, 2);
							...
							when set_mode =>
								if pressed = '1' then
									if key >= 1 and key <= 9 and cnt_tim = '0' then
										step <= key;
									end if;
									if key >= 0 and key <= 9 and cnt_tim = '1' then
										tim_sec_t := (tim_sec_t mod 10) * 10 + key;
										tim_min_t := (tim_min_t mod 10) * 10 + key;

										if sec_min = '0' then
											if tim_sec_t < 60 then
												tim_sec <= (tim_sec mod 10) * 10 + key;
											else
												tim_sec <= 50 + key;
											end if;
										elsif sec_min = '1' then
											if tim_min_t < 60 then
												tim_min <= (tim_min mod 10) * 10 + key;
											else
												tim_min <= 50 + key;
											end if;
										end if;
							...
							when init_mode =>
								case sw_d(2 to 4) is
									...
									when others =>
										...
										if init_fin_flag = '1' and tts_init_flag = '0' then
											case mode_tts is
												when idle =>
