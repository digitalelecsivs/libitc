...
architecture arch of v21_114_1_a is
	--state machine
	...
	signal num_pressed : unsigned(1 to 9) := (others => '0');--mode00中數字數是否顯示的旗標
	--計數旗標 & 暫存器 
		...
		process (clk, rst_n) begin
			if rst_n = '0' then
				...
				case mode_4 is
					...
					when UP_mode => dbg_b(0 to 3) <= not("001" & pressed_i);
					when DN_mode => dbg_b(0 to 3) <= not("111" & pressed_i);
					when others => dbg_b(0 to 3) <= not("000" & pressed_i);
				end case;
				case mode_tts is
					when idle => dbg_b(4 to 7) <= not("100" & '0');
					when play => dbg_b(4 to 7) <= not("010" & '0');
					when stop => dbg_b(4 to 7) <= not("001" & '0');
				end case;
			end if;
		end process;
	end block dbg_test;
						...
						case mode_1 is
							...
							when init_mode =>
								...
								if txt_cnt = 0 then
									x <= 8;
									y <= 4;
									if num_pressed(1) = '1' then
										text_color_array(1) <= red;
									elsif num_pressed(1) = '0'then
										text_color_array(1) <= blue;
									end if;
									if num_pressed(2) = '1' then
										text_color_array(2) <= red;
									elsif num_pressed(2) = '0'then
										text_color_array(2) <= blue;
									end if;
									if num_pressed(3) = '1' then
										text_color_array(3) <= red;
									elsif num_pressed(3) = '0'then
										text_color_array(3) <= blue;
									end if;
								...
								if txt_cnt = 1 then
									x <= 8;
									y <= 53 * 1 + 4;
									if num_pressed(4) = '1' then
										text_color_array(1) <= red;
									elsif num_pressed(4) = '0'then
										text_color_array(1) <= blue;
									end if;
									if num_pressed(5) = '1' then
										text_color_array(2) <= red;
									elsif num_pressed(5) = '0'then
										text_color_array(2) <= blue;
									end if;
									if num_pressed(6) = '1' then
										text_color_array(3) <= red;
									elsif num_pressed(6) = '0'then
										text_color_array(3) <= blue;
									end if;
								...
								if txt_cnt = 2 then
									x <= 8;
									y <= 53 * 2 + 4;
									if num_pressed(7) = '1' then
										text_color_array(1) <= red;
									elsif num_pressed(7) = '0'then
										text_color_array(1) <= blue;
									end if;
									if num_pressed(8) = '1' then
										text_color_array(2) <= red;
									elsif num_pressed(8) = '0'then
										text_color_array(2) <= blue;
									end if;
									if num_pressed(9) = '1' then
										text_color_array(3) <= red;
									elsif num_pressed(9) = '0'then
										text_color_array(3) <= blue;
									end if;
							...
							when set_mode =>
								if pressed = '1' then
									if key <= 9 and key >= 0 then
										cnt_max <= (cnt_max mod 100) * 10 + key;
									end if;
									case key is
										when 12 => up_dn <= not up_dn;
										when 14 => mode_1 <= ensure;
										...
										if fin_pro_flag = '1' then
											...
											if m_sel = 0 or m_sel = 1 then
												if up_dn = '0' then
													...
													tts_txt(0 to 27) <= cnt_cis & to_big(cnt_max - cnt_c(m_sel)) & cnt_lis & to_big(cnt_l(m_sel));
												end if;
												txt_len <= 28;
											else
												if up_dn = '0' then
													tts_txt(0 to 18) <= cnt_cis & to_string(cnt_c(m_sel), cnt_c(m_sel)'high, 16, 3) & cnt_lis & to_string(cnt_l(m_sel), cnt_l(m_sel)'high, 16, 2);
												elsif up_dn = '1' then
													tts_txt(0 to 18) <= cnt_cis & to_string(cnt_max - cnt_c(m_sel), cnt_c(m_sel)'high, 16, 3) & cnt_lis & to_string(cnt_l(m_sel), cnt_l(m_sel)'high, 16, 2);
												end if;
												txt_len <= 19;
											end if;
