...
architecture arch of v20_114_1_a is
	...
	signal txt_cnt : integer range 0 to 10;
	signal trigger_i : std_logic;
	signal trigger : std_logic;
	signal key_state : unsigned(1 to 9) := (others => '1');
	signal num_pressed : unsigned(1 to 9) := (others => '0');
	signal map_coord : l_coord_arr := ((0, 0), (0, 43), (0, 85), (53, 0), (53, 43), (53, 85), (106, 0), (106, 43), (106, 85));
	signal cnt_max : integer range 0 to 999 := 0;
	signal up_dn : std_logic := '1';

	-- constant aph : u8_arr_t(0 to 13) := (
	-- 	x"41", x"42", x"43", x"44", x"45", x"46",  x"48"
	-- );	--A  	 B		C	   D	  E 	 F 	     H	   十			 百
...
begin
		...
		process (clk, rst_n) begin
			if rst_n = '0' then
				...
				case mode_4 is
					...
					when UP_mode => dbg_b(0 to 3) <= not("010" & pressed_i);
					when tts_mode => dbg_b(0 to 3) <= not("001" & pressed_i);
					when init => dbg_b(0 to 3) <= not("111" & pressed_i);
					when others => dbg_b(0 to 3) <= not("000" & pressed_i);
				end case;
				case mode_tts is
	...
	Main_Process : block begin
		process (clk, rst_n)
			...
			variable str_c : u8_arr_t(0 to 3) := (x"30", x"30", x"30", x"48");
			variable str_l : u8_arr_t(0 to 1);
		begin
			if rst_n = '0' then
				case sw(6 to 7) is
					when"00" => mode_1 <= init;
				...
				case mode_t is
					when mode00 =>
						case mode_1 is
								...
								for i in 0 to 8 loop
									if i = 0 then
										if key_state(i + 1) = '0' then
											pic(i) := white;
										elsif key_state(i + 1) = '1' and num_pressed(i + 1) = '0' then
										...
										elsif key_state(i + 1) = '1' and num_pressed(i + 1) = '1' then
											pic(i) := to_data(l_paste(l_addr, white, l_map(data_array(i), blue, red), map_coord(i), 42, 53));
											addr_array(i) <= to_addr(l_paste(l_addr, white, l_map(data_array(i), blue, red), map_coord(i), 42, 53));
										end if;
									elsif i > 0 then
										if key_state(i + 1) = '0' then
											pic(i) := pic(i - 1);
											...
											pic(i) := to_data(l_paste(l_addr, pic(i - 1), l_map(data_array(i), blue, red), map_coord(i), 42, 53));
											addr_array(i) <= to_addr(l_paste(l_addr, pic(i - 1), l_map(data_array(i), blue, red), map_coord(i), 42, 53));
										end if;
									end if;
								end loop;
							when txt_mode => --文字顯示的初始化
								if pressed = '1' then
									case key is
										when 12 =>
											ena_tim <= '1';
								...
								if txt_cnt = 1 then
									...
									y <= 20;
									text_size <= 1;
									text_data(1 to 9) <= "EP3Q240C8";
									text_data(10 to 12) <= (others => character'val(32));
									font_start <= '1';
								end if;
								if txt_cnt = 2 then
									x <= 0;
								...
								if txt_cnt = 3 then
									...
									text_data(8 to 10) <= to_string(cnt_max, cnt_max'high, 10, 3);
									text_data(11 to 12) <= (others => character'val(32));
									font_start <= '1';
								end if;
								if font_busy = '1' then
									font_start <= '0';
							...
							when set_mode => --設定模式和cnt_max
								if pressed = '1' then
									case key is
										when 0 => cnt_max <= ((cnt_max mod 100)/10) * 100 + (cnt_max mod 10) * 10 + key;
										when 1 => cnt_max <= ((cnt_max mod 100)/10) * 100 + (cnt_max mod 10) * 10 + key;
										when 2 => cnt_max <= ((cnt_max mod 100)/10) * 100 + (cnt_max mod 10) * 10 + key;
										when 3 => cnt_max <= ((cnt_max mod 100)/10) * 100 + (cnt_max mod 10) * 10 + key;
										when 4 => cnt_max <= ((cnt_max mod 100)/10) * 100 + (cnt_max mod 10) * 10 + key;
										when 5 => cnt_max <= ((cnt_max mod 100)/10) * 100 + (cnt_max mod 10) * 10 + key;
										when 6 => cnt_max <= ((cnt_max mod 100)/10) * 100 + (cnt_max mod 10) * 10 + key;
										when 7 => cnt_max <= ((cnt_max mod 100)/10) * 100 + (cnt_max mod 10) * 10 + key;
										when 8 => cnt_max <= ((cnt_max mod 100)/10) * 100 + (cnt_max mod 10) * 10 + key;
										when 9 => cnt_max <= ((cnt_max mod 100)/10) * 100 + (cnt_max mod 10) * 10 + key;
										when 12 =>
											up_dn <= not up_dn;
										when 13 =>
											mode_1 <= set_mode;
							...
							when UP_mode =>
									...
									case key is
										...
										when 15 =>
											if trigger = '1' and m_sel /= 3 then
												if cnt_c(m_sel) < cnt_max then
													cnt_c(m_sel) := cnt_c(m_sel) + 1;
													cnt_l(m_sel) := cnt_c(m_sel)/30;
												elsif cnt_c(m_sel) = cnt_max then
													cnt_c(m_sel) := cnt_max;
													cnt_l(m_sel) := cnt_c(m_sel)/30;
												end if;
											end if;
								...
								if txt_cnt = 1 then
									...
									if m_sel = 0 then--xx
										...
										text_data(8 to 12) <= (others => character'val(32));
									elsif m_sel = 1 then--yy
										text_size <= 3;
										text_data(1) <= (character'val(32));
										text_data(2 to 4) <= to_string(cnt_c(m_sel), cnt_c(m_sel)'high, 10, 3);
										text_data(5 to 12) <= (others => character'val(32));
									elsif m_sel = 2 then--zz
										text_size <= 3;
										text_data(1) <= (character'val(32));
										text_data(2 to 4) <= to_string(cnt_c(m_sel), cnt_c(m_sel)'high, 16, 3);
										text_data(5 to 12) <= (others => character'val(32));
									elsif m_sel = 3 then
										text_size <= 1;
										text_data(1 to 7) <= "C  :000";
								...
								if txt_cnt = 2 then
									if m_sel = 0 then
										...
										y <= 96;
										text_size <= 3;
										text_data(1 to 2 ) <= (others => character'val(32));
										text_data(3 to 4) <= to_string(cnt_l(m_sel), cnt_l(m_sel)'high, 10, 2);
										text_data(5 to 12) <= (others => character'val(32));
									elsif m_sel = 2 then
										x <= 0;
										y <= 96;
										text_size <= 3;
										text_data(1 to 2) <= (others => character'val(32));
										text_data(3 to 4) <= to_string(cnt_l(m_sel), cnt_l(m_sel)'high, 16, 2);
										text_data(5 to 12) <= (others => character'val(32));
									elsif m_sel = 3 then
										x <= 0;
										y <= 64;
								...
								if txt_cnt = 3 then--null
									if m_sel = 0 or m_sel = 3 then
										x <= 0;
										y <= 48;
										text_size <= 1;
										text_data(1 to 12) <= (others => character'val(32));
										font_start <= '1';
							...
							when DN_mode =>
								...
								if txt_cnt = 1 then
									...
									if m_sel = 0 then--xx
										text_size <= 1;
										text_data(1 to 4) <= "C  :";
										text_data(5 to 7) <= to_string(cnt_max-cnt_c(m_sel), cnt_c(m_sel)'high, 10, 3);
										text_data(8 to 12) <= (others => character'val(32));
									elsif m_sel = 1 then--yy
										text_size <= 3;
										text_data(1) <= (character'val(32));
										text_data(2 to 4) <= to_string(cnt_max-cnt_c(m_sel), cnt_c(m_sel)'high, 10, 3);
										text_data(5 to 12) <= (others => character'val(32));
									elsif m_sel = 2 then--zz
										text_size <= 3;
										text_data(1) <= (character'val(32));
										text_data(2 to 4) <= to_string(cnt_max-cnt_c(m_sel), cnt_c(m_sel)'high, 16, 3);
										text_data(5 to 12) <= (others => character'val(32));
									elsif m_sel = 3 then
										text_size <= 1;
										text_data(1 to 7) <= "C  :000";
								...
								if txt_cnt = 2 then
									if m_sel = 0 then
										...
										text_size <= 1;
										text_data(1 to 5) <= "L  : ";
										text_data(6 to 7) <= to_string(cnt_max/30-cnt_l(m_sel), cnt_l(m_sel)'high, 10, 2);
										text_data(8 to 12) <= (others => character'val(32));
									elsif m_sel = 1 then
										x <= 0;
										y <= 96;
										text_size <= 3;
										text_data(1 to 2) <= (others => character'val(32));
										text_data(3 to 4) <= to_string(cnt_max/30-cnt_l(m_sel), cnt_l(m_sel)'high, 10, 2);
										text_data(5 to 12) <= (others => character'val(32));
									elsif m_sel = 2 then
										x <= 0;
										y <= 96;
										text_size <= 3;
										text_data(1 to 2) <= (others => character'val(32));
										text_data(3 to 4) <= to_string(cnt_max/30-cnt_l(m_sel), cnt_l(m_sel)'high, 16, 2);
										text_data(5 to 12) <= (others => character'val(32));
									elsif m_sel = 3 then
										x <= 0;
										y <= 64;
								...
								case mode_tts is
									...
									when play =>
										if finish_proc = '1' then
											...
											if up_dn = '1' then
												str_c(0 to 2) := to_string(cnt_c(m_sel), cnt_c(m_sel)'high, 16, 3);
												str_l := to_string(cnt_l(m_sel), cnt_l(m_sel)'high, 16, 2);

												if m_sel = 0 or m_sel = 1 then
													txt(0 to 27) <= cnt_cis & to_big(cnt_c(m_sel)) & cnt_lis & to_big(cnt_l(m_sel));
													txt_len <= 28; --7				7							7				7
												elsif m_sel = 2 then
													txt(0 to 19) <= cnt_cis & str_c & cnt_lis & str_l;
													txt_len <= 20;
												end if;--			7											7
											elsif up_dn = '0' then
												str_c(0 to 2) := to_string(cnt_max - cnt_c(m_sel), cnt_c(m_sel)'high, 16, 3);
												str_l := to_string(cnt_max/30 - cnt_l(m_sel), cnt_l(m_sel)'high, 16, 2);

												if m_sel = 0 or m_sel = 1 then
													txt(0 to 27) <= cnt_cis & to_big(cnt_max - cnt_c(m_sel)) & cnt_lis & to_big(cnt_max/30 - cnt_l(m_sel));
													txt_len <= 28; --7				7							7				7
												elsif m_sel = 2 then
													txt(0 to 19) <= cnt_cis & str_c & cnt_lis & str_l;
													txt_len <= 20;
