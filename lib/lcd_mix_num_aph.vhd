...
architecture arch of lcd_mix is
	...
	signal l_addr_mix : integer range 0 to 105_152;
	-- 新增：錯誤處理訊號
	-- 當字元不是 '0'-'9' 時，此訊號為 '1'
...
begin
	...
	l_addr_mix <= 2016 * data_y + data_x + first_px;
	...
	first_px <= 129 when character'pos(text_data(count + 1)) = 52 else
		...
		(character'pos(text_data(count + 1)) - 56) * 32 when (character'pos(text_data(count + 1)) >= 65) and (character'pos(text_data(count + 1)) <= 92) else
